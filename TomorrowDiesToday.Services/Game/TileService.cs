...
namespace TomorrowDiesToday.Services.Game
{
    public class TileService : ITileService
    {
        ...
        private void InitializeAllTiles()
        {
            ...
            allTiles.Add(new TileModel(TileType.CrashWallStreet, new TileStats(0,6,8,1), new TileStats(0,6,8,1)));
            allTiles.Add(new TileModel(TileType.DestroyIXPs, new TileStats(2,6,6,1), new TileStats(2,6,6,1))); 
            allTiles.Add(new TileModel(TileType.BringDownSatellites, new TileStats(2,2,7,4), new TileStats(2,2,7,4)));
            allTiles.Add(new TileModel(TileType.KidnapMilitaryAndPoliticalLeaders, new TileStats(3,6,3,3), new TileStats(3,6,3,3)));
            allTiles.Add(new TileModel(TileType.DeployNeurotoxin, new TileStats(4,3,6,2), new TileStats(4, 3, 6, 2)));
            allTiles.Add(new TileModel(TileType.BurgleFortKnox, new TileStats(1,8,4,2), new TileStats(1, 8, 4, 2)));
            allTiles.Add(new TileModel(TileType.SupplantMajorAfricanWarlords, new TileStats(8,4,1,2), new TileStats(8, 4, 1, 2)));
            allTiles.Add(new TileModel(TileType.TakeOverSouthAmericanCartels, new TileStats(4,8,1,2), new TileStats(4, 8, 1, 2)));
            allTiles.Add(new TileModel(TileType.HijackMajorWorldMediaOutlets, new TileStats(0,2,7,6), new TileStats(0, 2, 7, 6)));
            allTiles.Add(new TileModel(TileType.InfiltrateAsianIntelligenceAgencies, new TileStats(3,4,4,4), new TileStats(3, 4, 4, 4)));
            allTiles.Add(new TileModel(TileType.CorruptTheUN, new TileStats(0,2,5,8), new TileStats(0, 2, 5, 8)));
            allTiles.Add(new TileModel(TileType.CIABuilding, new TileStats(2,2,2,2), new TileStats(2, 2, 2, 2)));
            allTiles.Add(new TileModel(TileType.InterpolHQ, new TileStats(1,2,2,1), new TileStats(1, 2, 2, 1)));
